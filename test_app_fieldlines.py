...
# E. Field Line Controls
controls_fieldline = dmc.Paper(
    ...
    children=[
        ...
        dmc.Stack([
            ...
            # Controls for 1D Lines
            dmc.Group([
                dmc.Text("Number of Lines:", size="sm"),
                dmc.NumberInput(id="ctrl-fl-nlines", value=6, min=1, max=20, step=1, w=80)
            ], id="group-fl-nlines", style={"display": "none"}, mt="sm"),
            
            # Controls for Single Trace
            dmc.Group([
                dmc.Text("Transits:", size="sm"),
                dmc.NumberInput(id="ctrl-fl-transits", value=50, min=10, max=500, step=10, w=80)
            ], id="group-fl-transits", style={"display": "none"}, mt="sm"),
            
            dmc.Group([
                dmc.Text("Alpha Start:", size="sm"),
                dmc.NumberInput(id="ctrl-fl-alpha0", value=0.0, step=0.1, w=80)
            ], id="group-fl-alpha0", style={"display": "none"}, mt="sm"),
...
def update_visualization(view, filepath, var_1d, type_2d, var_2d, s_2d, var_3d, s_3d, bg_3d, reset_clicks, dark_mode, geo_count, 
                         fl_type, fl_s_idx, fl_nlines, fl_transits, fl_alpha0, fl_res,
                         meta, phi_state):
        ...
        if view == 'fieldline':
            ...
            if fl_type == '2d_modB':
                alpha_grid, zeta_grid, B_data = vmec.compute_field_line_properties(
                    s_idx=fl_s_idx, alpha_points=res, zeta_points=res, single_line=False
                )
                
                # Match plot_az_jax.py style:
                # Use Contour with coloring='fill' to simulate the filled background (like contourf/pcolormesh)
                # Overlay black contour lines (levels=20, linewidth=0.5)
                ...
                fig.update_layout(
                    title=f'|B| on Flux Surface s={fl_s_idx} (JAX Computed)',
                ...
                for idx in idx_list:
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        y=B_line,
                        mode='lines',
                        name=f'alpha={alpha_val:.2f}'
                    ))
                    
                fig.update_layout(
                    title=f'|B| along field lines (s={fl_s_idx})',
                    xaxis_title='Zeta [rad]',
                    yaxis_title='|B| [T]',
                    template=fig_template,
                    paper_bgcolor=paper_bg,
                ...
                zeta_grid, alpha_segments, B_data = vmec.compute_field_line_properties(
                    s_idx=fl_s_idx, zeta_points=res, n_transits=fl_transits, alpha0=fl_alpha0, single_line=True
                )
                ...
                # Construct continuous zeta axis for 1D plot
                ...
                fig = make_subplots(
                    rows=2, cols=1,
                    subplot_titles=(
                        f'Field Line Coverage ({len(alpha_segments)} transits)', 
                        '|B| along Field Line'
                    ),
                    vertical_spacing=0.15
                )
                
                # 1. Scatter Plot (Alpha vs Zeta)
                fig.add_trace(go.Scattergl(
                    x=Z_scatter,
                    y=A_scatter,
                    mode='markers',
                    marker=dict(
                        size=2, # Match s=2 in plot_az_jax
                        color=B_flat,
                        colorscale='Viridis',
                        showscale=True,
                        colorbar=dict(title='|B| [T]', len=0.45, y=0.78),
                        opacity=0.8 # Match alpha=0.8
                    ),
                ...
                # 2. 1D Line Plot (|B| vs Zeta_long)
                fig.add_trace(go.Scatter(
                    x=zeta_long,
                    y=B_flat,
                    mode='lines',
                    line=dict(width=0.5, color='#3bc9db'), # Match linewidth=0.5
                    name='|B| Trace'
                ), row=2, col=1)
